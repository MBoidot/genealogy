...
const spouseAngleDelta = 0.05; // small offset for spouses
root.each(node => {{
    if(node.data.id===1 || node.data.id===2) {{
        ...
        node.xOffset = node.data.isSpouse ? spouseAngleDelta : 0; // restore radial offset
    }}
}});
...
nodes.append("text")
    .attr("dy","0.31em")
    .text(d=> {{
        if(d.data.id===1 || d.data.id===2) return d.data.name; // full name for oldest ancestors
    ...
    .attr("transform", d => {{
        if(d.data.id===1) return `translate(0,-28)`;  // stacked at center
        if(d.data.id===2) return `translate(0,-12)`;  // stacked below ID1
